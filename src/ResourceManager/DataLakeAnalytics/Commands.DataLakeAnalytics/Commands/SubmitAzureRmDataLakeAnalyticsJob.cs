...
{
    [Cmdlet(VerbsLifecycle.Submit, "AzureRmDataLakeAnalyticsJob", DefaultParameterSetName = USqlJobParameterSetName), OutputType(typeof(JobInformation))]
    [Alias("Submit-AdlJob")]
    public class SubmitAzureDataLakeAnalyticsJob : DataLakeAnalyticsCmdletBase
    {
        ...
        internal const string ScopeJobWithScriptPath = "SubmitScopeJobWithScriptPath";

        private int _analyticsUnits = 1;
        private int _priority = 1000;
        private string _jobtype = UsqlType;

        // TODO: Remove this once other job types are enabled
        private const string UsqlType = "usql";
        ...
        [Parameter(ValueFromPipelineByPropertyName = true, ParameterSetName = USqlJobWithScriptPathAndPipeline, Position = 2,
            Mandatory = true, HelpMessage = "Path to the script file to submit.")]
        [Parameter(ValueFromPipelineByPropertyName = true, ParameterSetName = ScopeJobWithScriptPath, Position = 2,
            Mandatory = true, HelpMessage = "Path to the script file to submit.")]
        [ValidateNotNullOrEmpty]
        ...
        [Parameter(ValueFromPipelineByPropertyName = true, ParameterSetName = ScopeJobWithScriptPath, Position = 3,
            Mandatory = false,
            HelpMessage =
                "Optionally set the version of the runtime to use for the job. If left unset, the default runtime is used."
            )]
        ...
        [Parameter(ValueFromPipelineByPropertyName = true, ParameterSetName = ScopeJobWithScriptPath,
            Mandatory = false,
            HelpMessage =
                "The analytics units to use for this job. Typically, a higher degree of parallelism dedicated to a script results in faster script execution time."
            )]
        ...
        public string NebulaArgument { get; set; }
        ...
        public Hashtable CustomProperty { get; set; }
        ...
        public override void ExecuteCmdlet()
        {
            if (AnalyticsUnits < 1)
            {
                WriteWarning(Properties.Resources.InvalidAnalyticsUnits);
            }
                ...
                case UsqlType:
                    {
                        ...
                        if (ScriptParameter != null)
                        {
                            ...
                            foreach (DictionaryEntry param in ScriptParameter)
                            {
                                if (param.Value == null)
                                {
                                    throw new CloudException(string.Format(Properties.Resources.ScriptParameterValueIsNull,
                                        param.Key.ToString()));
                                }
                                ...
                                {
                                    throw new CloudException(string.Format(Properties.Resources.InvalidScriptParameterType,
                                        paramType.ToString()));
                                }
                            }
                        ...
                        {
                            var jobId = DataLakeAnalyticsClient.JobIdQueue.Count == 0
                                ? Guid.NewGuid()
                                : DataLakeAnalyticsClient.JobIdQueue.Dequeue();
                ...
                case ScopeType:
                    {
                        ...
                        JobInformation jobInfo = ScopeHelpers.Submit(this, powerShellDestinationPath);
                        WriteObject(jobInfo);
                        break;
                    }
                default:
